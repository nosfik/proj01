  ...
  if($security_code) {
	...
	$CodeExists = count($ex);
      ...
      if (count($ssu)>0 || count($sse)>0) {
		...
		$link = mysql_connect($db_server, $db_pechatnik_login, $db_pechatnik_password);
		mysql_query("use ".$db_pechatnik_database, $link);
		...
		$sql_add_customer_print = "INSERT INTO customer SET store_id = '0', firstname = '" . mysql_real_escape_string($fname) . "', note = '', transporter ='', send_email = '1', lastname = '" . mysql_real_escape_string($lname) . "', email = '" . mysql_real_escape_string($email) . "', telephone = '" . mysql_real_escape_string($nphone) . "', skype = '', salt = '" . $this -> db -> escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . mysql_real_escape_string(sha1($salt . sha1($salt . sha1($pass1)))) . "', newsletter = '0', customer_group_id = '1', ip = '" . $ip . "', status = '1', approved = '1', date_added = NOW()";
		
		$sql_add_address = "INSERT INTO address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this -> db -> escape($data['firstname']) . "', lastname = '" . $this -> db -> escape($data['lastname']) . "', city = '" . mysql_real_escape_string($city) . "', country_id = '" . (int)$country . "', zone_id = '0'";
		 
		 
		 
		mysql_query($sql_add_customer_print, $link);
		$customer_id = mysql_insert_id();
		mysql_query($sql_add_address, $link);
		$address_id = mysql_insert_id();
		 
	
    
    $address_id = $this -> db -> getLastId();
    mysql_query("UPDATE customer SET address_id = '" . (int)$address_id . "' WHERE customer_id = '" . (int)$customer_id . "'");

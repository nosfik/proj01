...
class ControllerAlbumAlbum extends Controller {
	...
	public function edit() {
		...
		if ($this -> customer -> isLogged()) {
			...
			$data = array('name' => $this -> request -> post['name'], 'description' => $this -> request -> post['description'], 'photo' => ($upload_file) ? $this -> request -> files["cover"]["name"] : '', 'album_id' => $this -> request -> post['album_id'], 'customer_id' => $this -> customer -> getId());
	...
	public function index() {
		...
		if (!$this -> customer -> isLogged()) {
			...
			foreach ($albums as $album) {
				...
				$this -> data['albums'][] = array('id' => $album['album_id'], 'name' => $album['name'], 'description' => $album['description'], 'photo' => $album['photo'], 'date' => $album['date'], 'size' => $album['size'], 'cover' => $cover);
			}

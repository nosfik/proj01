...
class ControllerAlbumContent extends Controller {
	...
	public function rotate() {
		...
		if ($this -> customer -> isLogged()) {
			...
			switch ($ext) {
				case 'png' :
					imagepng($rotate, $pathImage);
					break;
				case 'gif' :
					imagegif($rotate, $pathImage);
					break;
				case 'jpg' :
				case 'jpeg' :
				default :
					imagejpeg($rotate, $pathImage);
					break;
			}
	...
	public function edit() {
		...
		if (!$this -> customer -> isLogged()) {
			...
			if ($printmode && $paper && $red_eye && $white_border && $format && $cut && $color_correction) {
				...
				$config = array('album_id' => $album_id, 'color_correction' => $color_correction, 'cut_photo' => $cut, 'format' => $format, 'paper' => $paper, 'photo_id' => $photo_id, 'print_mode' => $printmode, 'red_eye' => $red_eye, 'white_border' => $white_border, 'customer_id' => $this -> customer -> getId());
				...
				if ($width && $height) {
					...
					switch ($ext) {
						case 'png' :
							$img_r = imagecreatefrompng($photo_src);
							break;
						case 'gif' :
							$img_r = imagecreatefromgif($photo_src);
							break;
						case 'jpg' :
						case 'jpeg' :
						default :
							$img_r = imagecreatefromjpeg($photo_src);
							break;
					}
					...
					switch ($ext) {
						case 'png' :
							imagepng($dst_r, $photo_new_src);
							break;
						case 'gif' :
							imagegif($dst_r, $photo_new_src);
							break;
						case 'jpg' :
						case 'jpeg' :
						default :
							imagejpeg($dst_r, $photo_new_src);
							break;
					}
				...
				if (!$album_id || !$photo_id) {
					...
					if (is_dir($albumDir)) {
						...
						$this -> data['photo'] = array('id' => $photo['album_photo_id'], 'prev' => (($photo['prev'] == null) ? '' : $this -> url -> link('album/content/edit', 'album=' . $album_id . '&photo=' . $photo['prev'], 'SSL')), 'next' => (($photo['next'] == null) ? '' : $this -> url -> link('album/content/edit', 'album=' . $album_id . '&photo=' . $photo['next'], 'SSL')), 'name' => $photo['photo_name'], 'format' => $photo['album_photo_format_id'], 'paper' => $photo['album_photo_paper_id'], 'printmode' => $photo['album_photo_printmode_id'], 'color_correction' => $photo['color_correction'], 'cut' => $photo['cut_photo'], 'white_border' => $photo['white_border'], 'red_eye' => $photo['red_eye'], 'path' => $albumDir . '/' . $photo['photo_name']);
	...
	private function makeSmallCopy($newDir, $fileName) {
		...
		switch ($ext) {
			case 'png' :
				$src = imagecreatefrompng($photo_file);
				break;
			case 'gif' :
				$src = imagecreatefromgif($photo_file);
				break;
			case 'jpg' :
			case 'jpeg' :
			default :
				$src = imagecreatefromjpeg($photo_file);
				break;
		}
		...
		switch ($ext) {
			case 'png' :
				imagepng($dst, $newDir . '/small_' . $fileName);
				break;
			case 'gif' :
				imagegif($dst, $newDir . '/small_' . $fileName);
				break;
			case 'jpg' :
			case 'jpeg' :
			default :
				imagejpeg($dst, $newDir . '/small_' . $fileName);
				break;
		}
		...
		if (!$this -> customer -> isLogged()) {
			...
			if (is_dir($albumDir)) {
				...
				foreach ($photos as $photo) {
					$this -> data['photos'][] = array('id' => $photo['album_photo_id'], 'path' => $albumDir . '/small_' . $photo['photo_name'], 'name' => $photo['photo_name']);
				}
			}

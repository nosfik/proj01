	 ...
	 <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
		...
		<?php foreach ($languages as $language) { ?>
          <div id="tab-language-<?php echo $language['language_id']; ?>">
			<table class="form">
				<tr>
					<td class="left">Картинка</td><td><input type="file" name="image" /><img src="<?php echo $this->url->link('extension/newsshare/image', 'newsshare_id='.$newsshare_id.'&token='.$token, 'SSL');?>" height="100px" /></td>
				</tr>
				<tr>
					<td class="left"><?php echo $text_title; ?></td>
					<td><input type="text" name="newsshare[<?php echo $language['language_id']; ?>][title]" value="<?php echo isset($newsshare[$language['language_id']]) ? $newsshare[$language['language_id']]['title'] : ''; ?>" /></td>
				</tr>
				<tr>
					<td><?php echo $text_description; ?></td>
					<td><textarea name="newsshare[<?php echo $language['language_id']; ?>][description]" id="description-<?php echo $language['language_id']; ?>"><?php echo isset($newsshare[$language['language_id']]) ? $newsshare[$language['language_id']]['description'] : ''; ?></textarea></td>
				</tr>
			</table>
		  </div>
		<?php } ?>
